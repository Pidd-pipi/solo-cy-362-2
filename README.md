...
```bash
cd backend && mvn test          # 后端：JUnit5 + 真实 HTTP 链（随机端口 + TestRestTemplate）+ H2，15 个用例
cd frontend && npm test         # 前端：Vitest + happy-dom，18 个用例
```

覆盖场景：

- **剧本候选同步**：剧本新增、编辑、停用、启用后，发布场次的候选列表无需刷新页面即同步（后端接口语义 + 前端共享 store 与双组件真实挂载两层验证）。
- **刷新竞态防护（前端共享 store，5 个固定时序）**：①首次加载在途时保存触发的刷新不复用保存前的旧请求；②同一时刻并发刷新合并为一次；③旧响应晚到不会覆盖较新结果；④旧请求失败被吞掉、由成功的新请求兑现等待者；⑤请求失败不清空已有数据且下一次刷新可恢复（含首次加载失败重试）。通过可手动控制延迟/乱序/失败的假后端精确复现。
- **报名主流程**：正常报名（剩余名额立即更新）、重复报名被拒、满员不能报名、取消后空位释放并可重报。
- **并发抢最后一个名额（走完整 HTTP 请求链）**：服务以随机端口真实启动，多线程经 `TestRestTemplate` 同时 POST 报名接口，8 人抢 1 个空位断言恰好 1 人 200、其余 7 人 400「已满员」、最终不超卖；另含同一玩家双击并发只成功一次、HTTP DELETE 取消释放空位后多人并发补位只进一人。
